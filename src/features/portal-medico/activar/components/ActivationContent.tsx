...
import { activateMedicalAccount, isMedicalActivationSuccessful, getMedicalActivationErrorMessage } from '../services/activation.service';

/**
 * Modal component for medical activation result
 */
...
const ActivationModal = ({ isOpen, isSuccess, message, onGoToLogin }: ActivationModalProps) => {
  const t = useTranslations('portal-medico.activation.modal');

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 portal-medico bg-opacity-50 flex items-center justify-center z-50 p-4">
      <MedicalCard variant="medical-portal" className="max-w-md w-full mx-4 p-4 sm:p-6 md:p-8">
          {/* Icon */}
...
/**
 * Main medical activation content component
 */
const ActivationContent = () => {
  const t = useTranslations('portal-medico.activation');
  ...
  useEffect(() => {
    ...
    const processActivation = async () => {
      ...
      try {
        const response = await activateMedicalAccount({ hashConfirmacion: codigo });
        
        if (isMedicalActivationSuccessful(response)) {
          setMessage(t('success-message'));
          setIsSuccess(true);
        } else {
          setMessage(getMedicalActivationErrorMessage(response));
          setIsSuccess(false);
        }
  ...
  const handleGoToLogin = () => {
    // setModalOpen(false);
    router.push({ pathname: '/portal-medico/login' });
  };
