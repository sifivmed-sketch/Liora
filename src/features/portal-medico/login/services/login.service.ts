...
/**
 * Interface for the medical doctor login request
 * Based on the API documentation structure
 */
interface LoginDoctorRequest {
  /** Doctor's username/email */
  usuario: string;
  /** Doctor's password */
  clave: string;
...
/**
 * Interface for the medical doctor login response
...
/**
 * Authenticates a medical doctor by calling the LoginUsuarios endpoint
 * @param request - Doctor login credentials including station ID
 * @returns Promise with the login response
 * @throws Error if the API call fails
 */
export const loginDoctor = async (
  request: LoginDoctorRequest
  ...
  const url = new URL(`${API_BASE_URL}/${API_MEDICAL_SEGMENT}/LoginUsuarios`);
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new Error(`Failed to login doctor: ${error.message}`);
    }
    throw new Error('Unknown error occurred while logging in doctor');
  }
};
 ...
 */
export const isLoginSuccessful = (response: LoginDoctorResponse): boolean => {
 ...
 */
export const getLoginErrorMessage = (response: LoginDoctorResponse): string | null => {
...
export type { LoginDoctorRequest, LoginDoctorResponse };
